...
-- ── Morocco (mirrors migrations/0017_morocco.sql) ────────────────────────
insert into public.countries (id, name_ar, name_en, iso_code, phone_code, currency_code, timezone, sort_order) values
...
-- Push foundation: device_tokens (owner-only) + push_outbox (service-role queue)
-- + enqueue-on-notification trigger (does NOT send; a backend worker delivers).
...
create table if not exists public.device_tokens (
  ...
  app_env text not null default 'development' check (app_env in ('development','preview','production')),
...
create trigger device_tokens_touch before update on public.device_tokens for each row execute function public.set_updated_at();
...
create trigger push_outbox_touch before update on public.push_outbox for each row execute function public.set_updated_at();
...
returns trigger language plpgsql security definer set search_path = public as $$
begin
  insert into public.push_outbox (notification_id, user_id) values (new.id, new.user_id)
...
-- ════════════════════════════════════════════════════════════════════════
-- Swap — 0022 account deletion (Google Play User Data policy / Apple 5.1.1(v))
-- ════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════
-- Swap — 0023 profiles PII hardening (block anon email/phone scraping)
-- ════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════
-- Swap — 0024 deletion-request scrub (no email → tombstone mapping survives)
-- ════════════════════════════════════════════════════════════════════════
