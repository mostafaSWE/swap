...
import { Button, Checkbox, Input, SegmentedControl, Select, Textarea } from "../../../src/components/ui";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!active) return;
        if (!loaded || loaded.owner_id !== data.user?.id) {
          setListing(null);
          return;
        }
        setListing(loaded);
    ...
    <>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.root}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          {countryId ? <Select label={t("newListing.fieldCity")} placeholder={t("common.selectCity")} value={cityId} onChange={setCityId} options={cities} /> : null}
          ...
          {!openToAny ? <Textarea label={t("newListing.fieldWanted")} value={wanted} onChangeText={setWanted} maxLength={500} /> : null}
          <Button label={paused ? t("editListing.statusPaused") : t("editListing.statusActive")} variant="secondary" onPress={() => setPaused((value) => !value)} fullWidth />
          ...
          {confirmDelete ? <View style={styles.deleteConfirm}>
            ...
            <View style={styles.deleteActions}><View style={styles.flex}><Button label={t("editListing.deleteYes")} variant="danger" onPress={remove} loading={busy} fullWidth /></View><View style={styles.flex}><Button label={t("common.cancel")} variant="secondary" onPress={() => setConfirmDelete(false)} fullWidth /></View></View>
          </View> : <Button label={t("editListing.delete")} variant="ghost" onPress={() => setConfirmDelete(true)} disabled={busy} fullWidth />}
        </ScrollView>
