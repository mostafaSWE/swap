...
/**
 * Platform responsibility disclaimer. Shown on listing details, before starting
 * a chat, and on the safety page. `variant="compact"` shows a one-line notice.
 */
export function SafetyDisclaimer({ variant = "full" }: { variant?: "full" | "compact" }) {
  ...
  if (variant === "compact") {
    return (
      <div className="flex items-start gap-2 rounded-card bg-amber-50 p-3 text-xs text-amber-800">
        <ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" aria-hidden />
        <p>{t("points.userResponsibility")}</p>
      </div>
    );
  }
