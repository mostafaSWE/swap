...
**Last worked on:** **Phase M3 — swap proposal negotiation loop done (Session 17, 2026-07-23)** — built + verified the **propose flow** (`ProposeSwapSheet` + `ListingPicker` → `createProposal`), the **`ProposalContextCard`** pinned in the conversation (role-flipped give/get, status badge, turn banner, turn-gated Accept/Counter/Decline/Withdraw), and live status via `subscribeToProposal`. **Verified cross-account on-device (@ahmed→@sara):** create → counter → status/turn transitions; inbox proposal badges render. Deal-closing (photo confirm/dispute/rating) deferred to M5 (needs `expo-image-picker`). **Committed locally only** (`42653b2`, not pushed). (Session 16: M3 messaging — inbox + Realtime chat, `f49da97`. Session 15: M3 auth, `c380945`. Session 14: M2 read surface COMPLETE. Session 12: M1 component kit.)

> ⚠ **Local commits not yet pushed:** `c380945` (M3 auth), `caf58da` (auth doc), `f49da97` (M3 messaging), `3161e48` (messaging doc), `42653b2` (M3 proposal loop) + this doc — the user asked to batch pushes (each push prompts a GitHub sign-in). Push everything at the end of M3. (Session 9: icons + Select/BottomSheet + forms. Session 8: inventory + expo-localization + primitives. Session 7: RTL-first i18n. Session 6: nodeLinker→isolated. Sessions 4–5: M0 + nav shell.)
...
- [~] Auth: **login** ✅ (email/username + password; `email_for_username` RPC; verified on-device signing in as @ahmed) · **logout** ✅ (auth-reactive Profile) · **register** ✅ (`signUp` + metadata, "check your email" screen; the DB `handle_new_user` trigger creates the profile). **TODO:** forgot/reset password, onboarding (country/city/avatar), and the email-confirm **deep-link into the app** (`emailRedirectTo` currently points at the web).
