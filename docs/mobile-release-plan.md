...
**Last worked on:** **Phase M2 — read surface COMPLETE (Session 14, 2026-07-23)** — Browse + Listing detail + Public profile (Session 13), then the **own-Profile tab** (auth-reactive via `supabase.auth`) + **Saved** screen + **Home category → filtered Browse** (Session 14). All verified on-device against **live data**, both locales — the **M2 exit criterion is met** (browse → open a listing → save → seller's public profile & reviews). Commits `91cd8ec`, `3002a39`. (Session 12: M1 component kit COMPLETE — ~22 components; `Icon` mirror fix; `ProposalContextCard` deferred to M3. Session 11: RTL direction invariant — native early-set + JS boot guard.) (Session 9: icons + Select/BottomSheet + forms. Session 8: inventory + expo-localization + primitives. Session 7: RTL-first i18n. Session 6: nodeLinker→isolated. Sessions 4–5: M0 + nav shell.)
