...
**Last worked on:** **Phase M2 — read surface (Session 13, 2026-07-23)** — built + verified on-device against **live data** (LTR+RTL): the **Browse** tab (search + category filter + sort + paginated `getListings`), the **Listing detail** screen (`app/listings/[id]`: images, meta, `WantedCard`, `SellerCard`, Save/Message/Report, view tracking), and the **Public profile** (`app/users/[username]`: `ProfileHeader` + their listings + reviews). Commit `91cd8ec`. (Session 12: M1 component kit COMPLETE — ~22 components, all verified LTR+RTL; `Icon` mirror fix; `ProposalContextCard` deferred to M3. Session 11: RTL direction invariant — native early-set + JS boot guard.) (Session 9: icons + Select/BottomSheet + forms. Session 8: inventory + expo-localization + primitives. Session 7: RTL-first i18n. Session 6: nodeLinker→isolated. Sessions 4–5: M0 + nav shell.)

**Next task (in order):**
1. **M2 read-surface CORE done + verified live** (Browse + Listing detail + Public profile, LTR+RTL). **M2 remainder:** Saved/wishlist screen (`getSavedListings`/`saveListing`/`isListingSaved`) · Following feed (`getFollowingListings`) · wire Home category chips → filtered Browse (pass `categoryId`) · a Categories index · the **own-Profile tab** (signed-in user's profile + their listings; real auth lands in M3). Then the static legal/info screens (link to the hosted URLs — Phase S). Keep the RTL+LTR verification discipline on every screen.
...
- [~] Home (featured + categories, tappable) ✅ · **Browse + search/filters/sort** ✅ · **Listing detail + view tracking** ✅ · **Public profile (listings + reviews)** ✅ — all verified on-device against **live data**, LTR+RTL (Session 13). **Remaining:** Saved/wishlist screen · Following feed · Home category chips → filtered Browse · Categories index · Own-profile tab.
