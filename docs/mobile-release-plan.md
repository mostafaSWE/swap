...
**Last worked on:** **Phase M1 — component inventory + RN primitive kit (Session 8, 2026-07-22)** — inventoried all ~130 `apps/web` components (5-agent workflow) to derive the kit; swapped locale detection `Intl`→**`expo-localization`** (native rebuild); built the **primitive layer** (`src/components/ui/`: Button/Card/Avatar/Badge/RatingStars/Chip/Divider/StatCell) and refactored CategoryGrid/ListingCard onto it. Typecheck 8/8; **verified LTR (`en`) + RTL (`ar`) on device**. (Session 7: RTL-first i18n foundation. Session 6: reverted `nodeLinker`→isolated + verified web/api. Session 5: nav shell. Session 4: closed M0.)

**Next task (in order):**
1. **Finish M1 kit (RTL-first, from the inventory roadmap):** icon system (`lucide-react-native` + `react-native-svg`, needs a native rebuild) → form primitives (`Input`/`Textarea`/`Checkbox`) → **`Select`→BottomSheet** + `Sheet` (unblocks filters/report/phone) → `Skeleton`/`SegmentedControl`/`Accordion`/`IconButton`/`ListRow`; then composites (`ItemArtwork`, `SwapPair`, `FeaturedCard`, `SellerCard`, `ProfileHeader`, `ChatBubble`, `ConversationCard`, `ProposalContextCard`, `NotificationBell`, `ReportDialog`, `FollowButton`, …). Verify each in RTL then LTR. **⚠ Bring the RTL-reload restart UX (screen + copy) to the human before building the in-app language switcher — don't decide it alone.**
...
- [x] **i18n + RTL** — DONE + **verified on device both directions** (Correction 2): device locale via **`expo-localization` `getLocales()`** (reliable cross-Android; replaced Hermes `Intl`) with **Arabic-first `ar` fallback** (non-ar/en → ar); the **full web `next-intl` catalog ported** (`src/i18n/{ar,en}.json`, 32 namespaces) + a `mobile` namespace, loaded by `src/i18n/index.ts` (nested `t()` + `{param}` interpolation + `EXPO_PUBLIC_LOCALE` test override); **`forceRTL`** — an Arabic UI mirrors to RTL even on an LTR device (screenshot-confirmed: header/hero right-aligned, chips RTL, tab bar reversed; `en` device → LTR). **⚠ TODO — flag to human, do NOT decide alone:** the production RTL-flip **reload is a designed restart experience**, not just `expo-updates` `reloadAsync()` — when the in-app language switcher lands, the user must be told the app is restarting to apply their language (a screen + copy decision). Also: ICU plural forms.
...
- [~] **RN component kit** — **derived from a full `apps/web` inventory** (5-agent workflow; ~130 web components mapped PORT/ADAPT/SKIP — see the roadmap below). **Primitive layer built** (`src/components/ui/`, all RTL-verified on device): `Button`, `Card`, `Avatar`, `Badge` (+`PROPOSAL_STATUS_TONE`/`STATUS_TONE` maps), `RatingStars`, `Chip`, `Divider`, `StatCell`; `Screen`/`EmptyState` already present; `ListingCard`/`CategoryGrid` refactored onto them. Reuses `@swap/ui` formatters. **TODO (from inventory, in order):** icon system (`lucide-react-native` + `react-native-svg`), `Input`/`Textarea`/`Checkbox`, **`Select`→BottomSheet** (heaviest — blocks filters/report/phone/country-city), `Sheet`/`BottomSheet`, `Skeleton`, `SegmentedControl`, `Accordion`, `IconButton`, `ListRow`, `StrengthMeter`; then composites (`ItemArtwork`, `SwapPair`, `FeaturedCard`, `WantedCard`, `CategoryCard/Carousel`, `SellerCard`, `ProfileHeader`, `ChatBubble`, `ConversationCard`, `ProposalContextCard`, `NotificationBell`, `ReportDialog`, `FollowButton`, `MessageButton`, `AvatarUpload`, …).
