...
**Last worked on:** **Phase M1 — RTL strategy locked + `reloadAsync` verified (Session 10, 2026-07-23)** — decided to **keep the native `forceRTL` flag** (D-7; JS-managed `direction` silently breaks nav/scroll/caret with no JS remedy); **verified on-device that `Updates.reloadAsync()` re-applies the flag on RN 0.86 Android — both directions + across a genuine cold start** → **Option A** approved for the M3 in-app switcher. Found the iOS `CFBundleLocalizations` gap; the first-launch mismatch options **await the user's pick** (D-7). `RatingStars` comment fixed. (Session 9: icons + Select/BottomSheet + forms. Session 8: inventory + expo-localization + primitives. Session 7: RTL-first i18n. Session 6: nodeLinker→isolated. Sessions 4–5: M0 + nav shell.)

**Next task (in order):**
1. **Finish M1 kit (RTL-first, zero rework — native-flag strategy locked, D-7):** remaining primitives `Skeleton`/`SegmentedControl`/`Accordion`/`ListRow`/`StrengthMeter`, then composites starting with **`SwapPair`** + **`ItemArtwork`** (brand-critical — most listing screens depend on them), then `FeaturedCard`/`WantedCard`/`CategoryCard`+`Carousel`/`SellerCard`/`ProfileHeader`/`ChatBubble`/`ConversationCard`/`ProposalContextCard`/`NotificationBell`/`ReportDialog`/`FollowButton`/`MessageButton`/`AvatarUpload`. Verify each in RTL then LTR. **⚠ Awaiting user pick (D-7):** the first-launch mismatch fix (align-native+splash *[rec]* / accept-glitch / en-fallback) — don't implement until picked; it's independent of the kit, so build the kit meanwhile. The RTL-reload restart UX is **now decided** (Option A; native flag + `reloadAsync` verified) — build it at M3.
...
- [x] **i18n + RTL** — DONE + **verified on device both directions** (Correction 2): device locale via **`expo-localization` `getLocales()`** (reliable cross-Android; replaced Hermes `Intl`) with **Arabic-first `ar` fallback** (non-ar/en → ar); the **full web `next-intl` catalog ported** (`src/i18n/{ar,en}.json`, 32 namespaces) + a `mobile` namespace, loaded by `src/i18n/index.ts` (nested `t()` + `{param}` interpolation + `EXPO_PUBLIC_LOCALE` test override); **`forceRTL`** — an Arabic UI mirrors to RTL even on an LTR device (screenshot-confirmed: header/hero right-aligned, chips RTL, tab bar reversed; `en` device → LTR). **RTL strategy DECIDED (Session 10 — see D-7):** stay on the **native `forceRTL` flag** (not JS-managed `direction`); `Updates.reloadAsync()` **empirically re-applies the flag on RN 0.86 Android — both directions + across a genuine cold start** (verified on-device) → **Option A** (branded "Switching language…" reload, both-language copy — not a spinner) approved for the in-app switcher at **M3**. iOS reload path unverified until M5 (research: the iOS Fabric reload bug was fixed in RN 0.79 < 0.86). Still pending: the **first-launch mismatch** pick (D-7) + ICU plural forms.
...
| **D-7** | Mobile RTL direction strategy + first-launch mismatch | ⚠ **PARTLY RESOLVED (Session 10, 2026-07-23).** **Strategy: keep the native `I18nManager.forceRTL` flag** (not JS-managed Yoga `direction`). The flag gives navigation/scroll/caret + all 3rd-party RTL for free; JS-managed silently breaks native-stack nav (`react-native-screens`, closed won't-fix), horizontal-scroll anchoring, and TextInput caret with **no JS remedy** — and converting the 18 primitives was never where the cost lived (5-agent research + a conversion audit). **`Updates.reloadAsync()` verified on-device (RN 0.86 Android) to re-apply the flag both directions + across cold start** → the in-app switcher uses **Option A** (branded reload, both-language copy) at M3; iOS reload unverified till M5 (research: fixed in 0.79). **STILL OPEN — user to pick the first-launch mismatch fix** (device locale ∉ {ar,en} → `ar` strings while native `isRTL=false` until restart; **self-heals on the 2nd launch** since forceRTL persists): **(C, rec)** align the native fallback to `ar` — iOS `CFBundleLocalizations:['ar','en']` + `CFBundleDevelopmentRegion:'ar'` — **+ a splash-reload for the Android LTR-non-en residual**; **(C-only)** accept the self-healing first-launch glitch; or **(B)** flip the JS fallback to `en` (diverges from web's `ar`). **Prerequisite either way: declare Arabic in the iOS localizations** — else every iOS Arabic user depends on forceRTL. Android is already correct (`android:supportsRtl='true'`). |
...
- [OPEN — must fix before the iOS build] **iOS does not declare Arabic as a supported localization** (`app.json` has no `CFBundleLocalizations`/`locales`). Until added, iOS selects the English base even on an Arabic device → native `I18nManager.isRTL=false` → **every iOS Arabic user depends on `forceRTL`** (restart), widening D-7's first-launch problem from a minority to most iOS users. Fix: `ios.infoPlist.CFBundleLocalizations:['ar','en']` (+ `CFBundleDevelopmentRegion:'ar'` to align the native fallback with the JS `ar` fallback). Part of D-7; iOS is untestable until M5. Android already correct (`android:supportsRtl='true'`). — 2026-07-23
...
- [NOTE] **`expo-updates` installed** (`~57.0.9`) — the mechanism for Option A's `reloadAsync()` (and future OTA). Inert until wired (no `updates` block in `app.json`; disabled in dev). Pre-positioned for the approved Option A (D-7). — 2026-07-23
