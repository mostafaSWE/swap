...
**Current phase:** **Phase M3 — Auth + Transactional Loop (IN PROGRESS).** M0/M1/M2 DONE & verified on-device. M3 so far: **auth** (login/logout/register) ✅, **messaging** (inbox + Realtime chat + start-from-listing) ✅, and the **swap proposal negotiation loop** (propose → context card → counter/accept/decline/withdraw) ✅ — all verified on the emulator against the live backend (proposal verified cross-account @ahmed→@sara). Next in M3: **deal-closing** (photo confirm/dispute/rating — needs `expo-image-picker`, overlaps M5), create/edit listing (also image picker), forgot/reset password, onboarding, and the in-app language switcher (D-7). **M1 component kit + M2 read surface complete.**
...
**Last worked on:** **Phase M3 — forgot-password + swap proposal loop done (Session 17–18, 2026-07-23).** Session 18: built + verified the **forgot-password request** screen (`resetPasswordForEmail`, always-"sent") + a "Forgot password?" login link (`309e62c`). Session 17: built + verified the **swap proposal negotiation loop** — `ProposeSwapSheet` + `ListingPicker` → `createProposal`, `ProposalContextCard` (role-flipped give/get, status badge, turn banner, turn-gated Accept/Counter/Decline/Withdraw), live status via `subscribeToProposal`; verified cross-account (@ahmed→@sara): create → counter → status/turn transitions; inbox badges render. Deal-closing deferred to M5 (needs `expo-image-picker`). **Committed locally only** (`42653b2`, `309e62c`, not pushed). (Session 16: M3 messaging, `f49da97`. Session 15: M3 auth, `c380945`. Session 14: M2 read surface COMPLETE.)

> ⚠ **Local commits not yet pushed:** `c380945` (M3 auth), `caf58da` (auth doc), `f49da97` (M3 messaging), `3161e48` (messaging doc), `42653b2` (M3 proposal loop), `0e687a4` (proposal doc), `309e62c` (forgot-password) + this doc — the user asked to batch pushes (each push prompts a GitHub sign-in). Push everything at the end of M3. (Session 9: icons + Select/BottomSheet + forms. Session 8: inventory + expo-localization + primitives. Session 7: RTL-first i18n. Session 6: nodeLinker→isolated. Sessions 4–5: M0 + nav shell.)

**Next task (in order):**
1. **M2 read surface COMPLETE + verified live** — Browse, Listing detail, Public profile, own-Profile tab, Saved, Home category filter; **exit criterion met.** Low-priority M2 leftovers: Following feed (deferred — unmounted on web too), a dedicated Categories index, static legal/info screens (→ Phase S). Otherwise **M2 → M3.**
...
- [~] Auth: **login** ✅ (email/username + password; `email_for_username` RPC; verified on-device signing in as @ahmed) · **logout** ✅ (auth-reactive Profile) · **register** ✅ (`signUp` + metadata, "check your email" screen; the DB `handle_new_user` trigger creates the profile) · **forgot-password** ✅ (request screen + login link → `resetPasswordForEmail`; always-"sent"; verified on-device). **TODO:** onboarding (country/city/avatar), the fully in-app password **reset** (set new password — needs the email **deep-link into the app**; today it resolves on the web reset page), and the email-confirm deep-link (`emailRedirectTo` currently points at the web).
- [ ] Create/edit/delete listing incl. image upload + reorder.
- [~] Messaging **inbox** ✅ + **Realtime chat** ✅ + **start conversation from a listing** ✅ (verified on-device — inbox renders @ahmed's 4 real conversations; send appends a "now" bubble + clears composer; `getOrCreateConversation` from listing detail). **TODO:** read receipts (mark-as-read on open), report-a-message (M4).
...
- [ ] Deal closing: both-sides photo confirm → complete; dispute. **(needs `expo-image-picker` — deferred to M5; the card already renders the status banner for agreed/awaiting_confirmation/completed/disputed.)**
