...
**Current phase:** **Phase M0 — code + build-infra DONE; exit criterion (on-device smoke) BLOCKED on a one-time admin action (Windows long paths).**
...
**What's DONE:** `apps/mobile` runs on **Expo SDK 57** (RN 0.86 / React 19.2.3, New Arch on); reaches the **live backend** via shared `@swap/api` (Supabase auth + **AsyncStorage persistence**, wired `SwapApiClient`, RLS reads, Realtime); the cold-start-logout root cause (`createSupabaseClient` ignoring a storage adapter) is **fixed**; bundle id **`me.justswap.app`**, name **"JustSwap"**. **Build infra:** Android env persisted (User scope); NDK 27.1.12297006 / CMake 3.22.1 / platform-36 / build-tools 36 installed; `expo-dev-client` + `expo prebuild` (android/ + ios/ gitignored — CNG); **`nodeLinker: hoisted`** added (pnpm's deep `.pnpm` paths broke the RN native build; hoisting fixed the third-party module compiles). **Typecheck 8/8 green** (incl. under hoisted); `expo-doctor` 19/20.

**⚠ BLOCKER — the local build needs Windows Long Paths (elevated/admin).** With hoisting, all third-party native modules compile; the build then fails at the **New-Architecture app-level codegen link**, where CMake mangles each C++ source's *full absolute path* into the object name → a **373-char object path** over Windows' hard **260-char** limit. Can't be shortened enough (even at `C:\S` ≈ 305). Fix = `LongPathsEnabled=1` (HKLM) — an **elevated registry write I cannot perform** (confirmed "access denied"). Exact command in Known gaps.

**Last worked on:** **Phase M0 build bring-up (Session 3, 2026-07-22)** — Android toolchain setup (SDK env, NDK/CMake/platform-36/build-tools), `expo-dev-client` + `expo prebuild`, diagnosing + fixing the pnpm path-length failure (`nodeLinker: hoisted`), and hitting the New-Arch codegen 260-char wall. Emulator + local NestJS API confirmed running; the M0 harness has an env-gated **headless auto-smoke** ready to log `[M0]` results the moment the build runs. (Session 2 landed the M0 code: SDK 51→57, the `createSupabaseClient` fix, `supabase.ts`/`api.ts` wiring, polyfills, `app.json` identity, the connectivity harness.)

**Next task (in order):**
1. **YOU (one-time, elevated):** enable Windows Long Paths — see the command in Known gaps — then reopen the terminal and tell me. I'll re-run `expo run:android`; the harness auto-smoke then verifies auth / persistence-across-restart / REST `/me` / Realtime and closes M0. *(If long paths still won't build — RN New-Arch on Windows is finicky — the reliable fallback is an **EAS** dev build, which you wanted to avoid; I'll flag before switching.)*
2. **Phase M1** — expo-router tabs + nested stacks, i18n + RTL (replace the hardcoded `locale="ar"`), theme provider, RN component kit. (M1 code can proceed and be typecheck-verified even before the device smoke.) Then M2 → M6.

> **Reality check:** M0 code + backend wiring are done and green — the mobile app is a real authenticated client, and all third-party native modules now compile locally. The ONLY thing between here and the app running on the emulator is a one-line admin registry change (Windows long paths). Everything else (SDK/NDK/emulator/local API/deps/harness) is staged. Still zero end-user feature parity (home is the M0 harness). No developer account is needed until the **iOS hard stop at M5**.
...
**Exit criterion:** **Code + build-infra complete & statically verified** (typecheck 8/8 incl. under hoisted; expo-doctor 19/20; all third-party native modules compile). **On-device smoke BLOCKED** on Windows long paths (see Known gaps + RESUME) — the New-Arch codegen link exceeds the 260-char path limit. Once long paths is enabled and `expo run:android` completes, the harness auto-smoke logs `[M0]` boot-session (persistence) / rls / rest / realtime → M0 closes.
...
- [BLOCKER — human/admin] **Local Android build needs Windows Long Paths enabled.** RN's New-Architecture app-level codegen mangles each C++ source's full absolute path into the object filename → object paths ~**373 chars**, past Windows' hard **260** limit. `nodeLinker: hoisted` shortened the third-party module paths (fixing those compiles) but can't fix the codegen link (even relocating the repo to `C:\S` ≈ 305). Fix requires `LongPathsEnabled=1`, an elevated HKLM write I can't perform (confirmed "access denied"). **YOU run in an ELEVATED PowerShell (Run as Administrator):** `New-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem' -Name 'LongPathsEnabled' -Value 1 -PropertyType DWord -Force` (optionally also `git config --global core.longpaths true`), then close & reopen the terminal (reboot if it still fails). If the RN New-Arch toolchain still isn't fully long-path-aware (a known Windows rough edge), the reliable fallback is an **EAS** dev build (Linux cloud, no 260 limit) — flagged, not auto-taken. — 2026-07-22
- [INFRA] **pnpm `nodeLinker: hoisted`** (in `pnpm-workspace.yaml`) is required for RN native builds — pnpm's default deep `.pnpm/<pkg>@<ver>_<hash>/…` paths overflow CMake object paths. Cross-platform-safe, doesn't change `pnpm-lock.yaml` (CI `--frozen-lockfile` OK), all 8 workspaces typecheck green under it. Also set: `apps/mobile/android/gradle.properties` `reactNativeArchitectures=x86_64` for fast emulator builds (regenerated by prebuild; production/EAS uses all ABIs). — 2026-07-22
