...
**Last worked on:** **Phase M1 — RTL-first i18n foundation (Session 7, 2026-07-22)** — fixed the locale fallback to **ar** (Arabic-first) for non-ar/en devices; ported the full web `next-intl` catalog into `src/i18n/{ar,en}.json` + `index.ts` (nested `t()`, `ar` fallback, `EXPO_PUBLIC_LOCALE` override) + a `mobile` namespace; rewired all tab screens/components to catalog keys; implemented **`forceRTL`**. **Verified full RTL on the emulator** (Arabic UI mirrored — hero right-aligned, tab bar reversed; screenshot). Typecheck 8/8. (Session 6: reverted `nodeLinker`→isolated, verified web/api, pushed. Session 5: nav shell. Session 4: closed M0.)

**Next task (in order):**
1. **Finish M1 (RTL-first):** build the remaining component kit **in RTL from the start** — ChatBubble, ProposalContextCard, BottomSheet, ImageGallery, RatingStars, Avatar, Skeleton, form primitives (logical `start/end`, verify each in RTL then LTR). Then the production RTL-flip reload (`expo-updates` `reloadAsync`) + `expo-localization` (device locale) + nested stacks / auth screens outside the tabs.
...
- [~] **i18n + RTL** — foundation DONE + **RTL verified on device** (Correction 2): JS-only device-locale via `Intl` with **Arabic-first `ar` fallback** (non-ar/en → ar); the **full web `next-intl` catalog ported** (`src/i18n/{ar,en}.json`, 32 namespaces) + a `mobile` namespace, loaded by `src/i18n/index.ts` (nested `t()` + `{param}` interpolation + `EXPO_PUBLIC_LOCALE` test override); **`forceRTL`** implemented — an Arabic UI mirrors to RTL even on an LTR device (**screenshot-confirmed**: header/hero right-aligned, category chips RTL, tab bar reversed). TODO: production RTL-flip **reload** (`expo-updates` `reloadAsync`, lands with the in-app language switcher — a JS reload can't flip the native axis) + `expo-localization` (device locale) + ICU plurals. Build remaining components RTL-first.
