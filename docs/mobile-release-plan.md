...
**Last worked on:** **Phase M3 — auth started (Session 15, 2026-07-23)** — built + verified **Login** (email/username + password → `signInWithPassword`; verified on-device signing in as @ahmed) and **Register** (`signUp` + metadata; the DB `handle_new_user` trigger creates the profile; email-confirm screen); wired the Profile "Sign in" → `/login`. **Committed locally only** (`c380945`, not pushed — user is batching pushes to avoid repeated GitHub sign-in). (Session 14: M2 read surface COMPLETE — Browse/detail/profile/own-profile/Saved, exit criterion met. Session 12: M1 component kit. Session 11: RTL direction invariant.)

> ⚠ **Local commits not yet pushed:** `c380945` (M3 auth) + this doc — the user asked to batch pushes (each push prompts a GitHub sign-in). Push everything at the end of M3. (Session 9: icons + Select/BottomSheet + forms. Session 8: inventory + expo-localization + primitives. Session 7: RTL-first i18n. Session 6: nodeLinker→isolated. Sessions 4–5: M0 + nav shell.)
