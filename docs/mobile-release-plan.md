...
**Current phase:** **Phase M3 — Auth + Transactional Loop (IN PROGRESS).** M0/M1/M2 DONE & verified on-device. M3 so far: **auth** (login/logout/register) ✅ and **messaging** (inbox + Realtime chat + start-from-listing) ✅ — both verified on the emulator against the live backend. Next in M3: **proposal flow** (`ProposalContextCard` + propose/counter/accept), then create/edit listing (needs `expo-image-picker`), forgot/reset password, onboarding, and the in-app language switcher (D-7). **M1 component kit + M2 read surface complete.**
...
**Last worked on:** **Phase M3 — messaging done (Session 16, 2026-07-23)** — built + verified the **messaging inbox** (`app/(tabs)/messages`, client-side `fetchConversations`) and **Realtime conversation** (`app/messages/[id]`: live messages, own/other bubbles, `subscribeToMessages`, optimistic send); listing-detail `MessageButton` opens/creates the conversation. **Verified on-device (@ahmed):** 4 real conversations, correct own/other bubbles, send appends a "now" bubble. **Committed locally only** (`f49da97`, not pushed). (Session 15: M3 auth — Login/Register verified, `c380945`. Session 14: M2 read surface COMPLETE. Session 12: M1 component kit. Session 11: RTL direction invariant.)

> ⚠ **Local commits not yet pushed:** `c380945` (M3 auth), `caf58da` (auth doc), `f49da97` (M3 messaging) + this doc — the user asked to batch pushes (each push prompts a GitHub sign-in). Push everything at the end of M3. (Session 9: icons + Select/BottomSheet + forms. Session 8: inventory + expo-localization + primitives. Session 7: RTL-first i18n. Session 6: nodeLinker→isolated. Sessions 4–5: M0 + nav shell.)
